...
public:
    HelperThread(bool* runFlag, bool* extremes, std::function<bool(const QString &, std::istringstream&)> sendFun, Extreme* max, Extreme* min, const QString* fw, const QString* type)
        :   _sendFun(sendFun), _extremes(extremes), _maximum(max), _minimum(min), _runFlag(runFlag), _fwVersion(fw), _type(type)
    {}
